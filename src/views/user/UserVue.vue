...
import { userInfoService, userInfoGetNameService } from '@/api/user.js'; // 引入 API 函数
...
const isSearching = ref(false); // 新增标志位，用于区分是否在搜索状态
...
const loadUsers = async () => {
    ...
    try {
        let response;
        if (isSearching.value && searchForm.username) {
            const params = {
                username: searchForm.username,
                page: pageNum.value,
                size: pageSize.value
            };
            response = await userInfoGetNameService(params);
        } else {
            const params = {
                page: pageNum.value,
                size: pageSize.value,
            ...
            response = await userInfoService(params);
        }
